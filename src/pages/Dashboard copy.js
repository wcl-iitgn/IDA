  ...
  useEffect(() => {
    if (selectedSession && selectedYear) {
      const fetchData = async () => {
        try {
          setLoading(true)
          const droughtData = require(`../assets/data/Drought_Data/${selectedSession}.json`);
          const droughtArea = require(`../assets/data/Drought_Data/Area.json`);
          const droughtIntensity = require(`../assets/data/Drought_Data/Intensity.json`);
          ...
          if (filteredDroughtArea.length > 0) {
            ...
            if (selectedDroughtValue !== undefined) {
              ...
              // Handle the case where the selectedSession value is not available
              setDroughtArea(null);
            }
          } else {
            // Handle the case where no matching data for the selected year is found
            setDroughtArea(null);
          }
